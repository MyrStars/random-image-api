...
在线配置系统参数，无需手动编辑文件。配置保存在数据库中，重启后自动加载。
...
|--------|------|--------|
| 服务端口 | HTTP 监听端口 | 3100 |
| 公开访问地址 | 用于生成 API 地址 | http://localhost:3100 |
| CORS 允许来源 | 跨域访问白名单，逗号分隔 | * |
...
| 管理员用户名 | 登录账号 | — |
| 管理员密码 | 登录密码 | 修改后需重新登录 |
| JWT 密钥 | Token 签名密钥 | 修改后所有用户需重新登录 |
| 加密密钥 | 存储源密钥的加密密钥 | ⚠️ 修改后已有存储源密钥无法解密！ |
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 上传大小限制 | 单文件最大 MB 数 | 50 MB |
| 单次上传文件数上限 | 一次最多上传几张 | 20 |
| 图片缩放最大尺寸 | API 缩放请求的最大宽/高 | 4096 px |

#### 高级配置

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 数据库路径 | SQLite 文件位置 | ./data/images.db |
| 缓存最大条目数 | 内存缓存上限 | 500 |
| 自动保存间隔 | 数据库自动保存频率 | 30 秒 |
| 公开 API 频率限制 | 每分钟请求上限 | 120 次 |
| 登录频率限制 | 每分钟登录上限 | 10 次 |
...
首次启动前需要配置 `.env` 文件（从 `.env.example` 复制）：
